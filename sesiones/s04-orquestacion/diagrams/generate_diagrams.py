...
def save_fig(fig, name):
    fig.savefig(
        f"{name}.png", dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor(), edgecolor="none"
    )
    plt.close(fig)
    print(f"  -> {name}.png")
...
def diagram_01_the_problem():
    ...
    # X mark for failure
    ax.text(
        3.7, 4.2, "FALLO!", fontsize=12, color=COLORS["error"], fontweight="bold", style="italic"
    )
    ...
    ax.text(5, 0.5, "VS", fontsize=18, ha="center", fontweight="bold", color=COLORS["gray"])

    # --- Lado derecho: Con orquestacion ---
    ax.text(
        7.5,
        8.5,
        "Con Orquestacion (Prefect)",
        fontsize=16,
        ha="center",
        fontweight="bold",
    ...
    draw_box(
        ...
        3.5,
        0.7,
        "Dashboard: estado, logs, metricas",
        COLORS["light_blue"],
        text_color=COLORS["prefect_dark"],
...
def diagram_02_five_pillars():
    ...
    for x, title, desc, color, bg_color in pillars:
        ...
        ax.text(
            x,
            4.2,
            str(pillars.index((x, title, desc, color, bg_color)) + 1),
            fontsize=14,
            ha="center",
            va="center",
            fontweight="bold",
    ...
    ax.text(
        ...
        fontfamily="sans-serif",
        style="italic",
        bbox={"boxstyle": "round,pad=0.5", "facecolor": COLORS["light_gray"], "edgecolor": "none"},
    )
...
def diagram_03_flow_and_task():
    ...
    ax.text(
        5,
        2.7,
        "Lo que Prefect agrega automaticamente:",
        fontsize=11,
        ha="center",
        color=COLORS["prefect_dark"],
        fontweight="bold",
...
def diagram_04_task_graph():
    ...
    ax.text(
        5,
        8.2,
        "@flow: duration_prediction_pipeline",
        fontsize=13,
        ha="center",
        fontweight="bold",
        color=COLORS["prefect_dark"],
    ...
    for _key, (x, y, text, color) in tasks.items():
        ...
        lines = text.split("\n")
        ax.text(x, y + 0.15, lines[0], fontsize=10, ha="center", color="white", fontweight="bold")
        ax.text(x, y - 0.2, lines[1], fontsize=8, ha="center", color="#E0E0E0")
    ...
    ax.text(
        5,
        1.2,
        "Cada flecha = datos que pasan de un task al siguiente.\n"
...
def diagram_05_states():
    ...
    for x, y, name, color, desc in states:
        ...
        ax.text(x, y + 0.2, name, fontsize=10, ha="center", fontweight="bold", color="white")
    ...
    ax.text(
        5,
        8.5,
        "Cada @flow y @task pasa por estos estados",
        fontsize=12,
        ha="center",
        color=COLORS["gray"],
        style="italic",
    )
    ax.text(7.5, 5, "Exito", fontsize=10, color=COLORS["success"], fontweight="bold", rotation=30)
    ax.text(7.5, 4.1, "Error", fontsize=10, color=COLORS["error"], fontweight="bold", rotation=-30)
    ax.text(3.5, 2.2, "retry", fontsize=10, color=COLORS["orange"], fontweight="bold", rotation=70)
...
def diagram_06_retries():
    ...
    for i, line in enumerate(code_lines):
        ...
        ax.text(0.5, 8.1 - i * 0.35, line, fontsize=8.5, fontfamily="monospace", color=color)
    ...
    for x, label, color, status, success in attempts:
        ...
        draw_box(
            ...
            1.0,
            "",
            color if not success else COLORS["success"],
            alpha=0.15,
            border_color=color,
        )
        ax.text(x, 3.2, status, fontsize=8, ha="center", color=color, fontweight="bold")

        ax.text(x, 5.0, label, fontsize=9, ha="center", color=color, fontweight="bold")
    ...
    for i in range(3):
        ...
        ax.text((x1 + x2) / 2, 4.8, "10s", fontsize=8, ha="center", color=COLORS["orange"])

    # Bottom explanation
    ax.text(
        5,
        1.5,
        "Sin retries: tu pipeline falla y te enteras horas despues.\n"
...
def diagram_07_caching():
    ...
    draw_box(ax, 7.5, 4.4, 2.5, 0.8, "entrenar_modelo (120 seg)", COLORS["purple"], fontsize=9)
    ...
    ax.text(
        5,
        1.5,
        "# Si los datos no cambian, no los recalcula",
        fontsize=9,
        fontfamily="monospace",
        color="#78909C",
        ha="center",
...
def diagram_08_deployment():
    ...
    for x, y, title, code, desc, color in levels:
        # Card
        draw_box(
            ax, x, y, 2.1, 2.5, "", COLORS["white"], border_color=color, style="round,pad=0.15"
        )
        ax.text(x, 8.0, title, fontsize=10, ha="center", fontweight="bold", color=color)
        ...
        ax.add_patch(code_bg)
        ax.text(x, 6.9, code, fontsize=6.5, fontfamily="monospace", color="#4FC3F7", ha="center")
    ...
    ax.text(
        5,
        4.0,
        "Expresiones Cron: Cuando se ejecuta?",
        fontsize=14,
        ha="center",
        fontweight="bold",
        color=COLORS["prefect_dark"],
...
def diagram_09_architecture():
    ...
    # --- Your Code (left) ---
    ax.text(2, 9, "Tu Codigo", fontsize=13, ha="center", fontweight="bold", color=COLORS["gray"])

    draw_box(ax, 2, 7.5, 2.5, 1.2, "", COLORS["light_gray"], border_color=COLORS["gray"])
    ax.text(
        2, 7.8, "@flow + @task", fontsize=11, ha="center", fontweight="bold", color=COLORS["gray"]
    )
    ax.text(2, 7.2, "pipeline.py", fontsize=9, ha="center", color=COLORS["gray"], style="italic")

    # --- Prefect Server (center) ---
    server_bg = FancyBboxPatch(
        (3.8, 3.5),
        4.4,
        5.5,
        boxstyle="round,pad=0.3",
    ...
    # --- UI (right) ---
    ax.text(
        9, 9, "Dashboard UI", fontsize=13, ha="center", fontweight="bold", color=COLORS["purple"]
    )

    draw_box(ax, 9, 7.5, 2.0, 1.2, "", COLORS["light_purple"], border_color=COLORS["purple"])
    ax.text(
        9,
        7.8,
        "localhost:4200",
    ...
    ax.text(9, 7.2, "Flows, Runs,\nLogs, Artifacts", fontsize=8, ha="center", color=COLORS["gray"])
    ...
    draw_box(ax, 3, 1.5, 3.0, 1.2, "", COLORS["light_teal"], border_color=COLORS["teal"])
    ax.text(
        3,
        1.8,
    ...
    ax.text(
        3, 1.2, "Todo en tu maquina\nSQLite local", fontsize=8, ha="center", color=COLORS["gray"]
    )
    ...
    draw_box(ax, 7.5, 1.5, 3.0, 1.2, "", COLORS["light_blue"], border_color=COLORS["prefect_blue"])
    ax.text(
        7.5,
        1.8,
        "app.prefect.cloud",
...
def diagram_10_ml_pipeline():
    ...
    steps_row1 = [
        ...
        (5, 7.5, "Validar\nDatos", "Volumen +\nNulos", COLORS["teal"], "validate_data()"),
        (
            8.3,
            7.5,
            "Crear\nFeatures",
    ...
    steps_row2 = [
        ...
        (5, 5.3, "Entrenar\nModelo", "XGBoost\nbest_params", COLORS["purple"], "train_model()"),
        (
            8.3,
            5.3,
            "Registrar\nen MLflow",
    ...
    for steps_row in [steps_row1, steps_row2]:
        for x, y, title, desc, color, func in steps_row:
            ...
            ax.text(x, y + 0.35, title, fontsize=10, ha="center", fontweight="bold", color="white")
            ax.text(x, y - 0.15, desc, fontsize=8, ha="center", color="#E0E0E0")
            ax.text(
                x, y - 0.55, func, fontsize=7, ha="center", fontfamily="monospace", color="#B0BEC5"
            )
    ...
    integrations = [
        (
            ...
            2.5,
            "NYC TLC Dataset",
            "Datos publicos de taxis\nen formato Parquet",
            COLORS["prefect_blue"],
        ),
        (5, 2.5, "Prefect Dashboard", "Estado, logs,\nartefactos visuales", COLORS["purple"]),
        (8, 2.5, "MLflow Tracking", "Metricas, modelo,\nModel Registry", COLORS["orange"]),
    ]

    for x, y, title, desc, color in integrations:
        draw_box(
            ax, x, y, 2.6, 1.5, "", COLORS["white"], border_color=color, style="round,pad=0.15"
        )
        ax.text(x, y + 0.35, title, fontsize=10, ha="center", fontweight="bold", color=color)
        ax.text(x, y - 0.2, desc, fontsize=8, ha="center", color=COLORS["gray"])
...
def diagram_11_prefect_mlflow():
    ...
    ax.text(7.5, 7.7, "MLflow", fontsize=16, ha="center", fontweight="bold", color=COLORS["orange"])
    ax.text(
        7.5,
        7.2,
        "Experiment Tracking",
    ...
    ax.text(
        5, 4.7, "+", fontsize=30, ha="center", va="center", fontweight="bold", color=COLORS["gray"]
    )
    ...
    ax.text(
        5,
        0.8,
        "Prefect responde: CUANDO y COMO se ejecuta el pipeline\n"
