...
| Commitear `.env`, `data/raw/*.parquet` o `model.pkl` | secretos en el historial (que siguen ahí después de borrar el archivo) y un repositorio que crece sin límite: cada versión de un binario de 40 MB se queda para siempre | `.env.example` + LFS para los binarios que **sí** se versionan + `make data` para descargar el resto |
